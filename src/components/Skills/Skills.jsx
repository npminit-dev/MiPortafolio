...
export default function Skills() {
  ...
  const [skSections, setSkSections] = useState(skills)
  const [selected, setSelected] = useState(3)
  const skMaskInner = useRef()
  const skContainer = useRef()

  useEffect(() => {
    skMaskInner.current.style.transform = 
    selected === 1 ? 'translateX(585px)' : 
    selected === 2 ? 'translateX(286px)' :
  ...
  return (
    <section className='skills' ref={skContainer}>
      ...
      <div className='skills__mask'>
        ...
        { 
          selected === 1 ? <FcIdea className='backgroundicon__icon idea'/> : 
          selected === 2 ? <PiPersonArmsSpread className='backgroundicon__icon person'/> :
          selected === 3 ? <BiLogoReact className='backgroundicon__icon react'/> :
          selected === 4 ? <BiLogoNodejs className='backgroundicon__icon node'/> :
          selected === 5 ? <BsFillDatabaseFill className='backgroundicon__icon db'/> : <></>
        }
        </div>
        <div 
          className='skills__mask__inner'
          ref={skMaskInner}>
        {
        skSections.map((sk, i) => 
          <SkillList
      ...
      skSections.map((_, i) => 
       <div key={uuidv4()}>
          <span 
            className={`${i + 1 === selected ? 'skills__selectcount__item selected' : 'skills__selectcount__item unselected'}`}
          ></span>
       </div>
        ) 
      }
