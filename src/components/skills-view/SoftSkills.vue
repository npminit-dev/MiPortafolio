...
const startAnimations = () => {
  ...
  splits = Array.from(paragraphs).map((p) => new SplitText(p, { type: "lines,words" }));
...
<template>
  ...
  >
    <div
      id="black-background"
      class="absolute h-[200vh] bg-void-975 w-screen -z-[200] -top-1/2 left-0 pointer-events-none"
    />
    <SolarSystem id="solar-system" :height="1500" class="absolute -z-50 opacity-40" />
    <div class="h-full w-full flex items-center justify-center">
      <p
        class="absolute font-display text-ghost-200 text-[21px] text-center leading-tight max-w-[600px] shadowed-p"
      >
        {{
          $t(
            "Subject D-095 exhibits heightened interpersonal sensitivity, consistently registering external signals with precision. Within collaborative environments, D-092 maintains sustained active-listening patterns, enabling rapid assimilation of contextual data. Observed interactions indicate a stable tendency toward cooperative alignment rather than competitive escalation, optimizing group coherence."
          )
        }}
      </p>
      <p
