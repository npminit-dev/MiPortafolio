...
<template>
  <button
    ...
    ref="container"
    @click="handleToggle"
    class="relative flex items-center justify-center w-10 h-10 p-2 rounded-full border-[1px] transition-all duration-300 cursor-pointer hover:scale-110 active:scale-95 overflow-hidden bg-void-950 z-10"
    :class="st.isSoundEnabled ? 'border-ghost-100' : 'border-ghost-300/40'"
  >
    <div class="w-full h-full overflow-hidden flex items-center justify-center">
      ...
      >
        <polyline
          id="sound-switcher-wave"
          ref="wave"
          fill="none"
          :stroke="
            st.isSoundEnabled ? 'var(--color-ghost-100)' : 'var(--color-ghost-300)'
          "
          stroke-width="1"
